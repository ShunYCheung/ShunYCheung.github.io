...
k=(y-\mu(\theta))^T(C^{-1})(y-\mu(\theta))
\end{align}$$

and thus we can rewrite our integral

$$\begin{align}
\frac{1}{(2\pi\Lambda)(2\pi C)}\exp(-\frac{k}{2})\int^\infty_{-\infty}\exp\left(-\frac{1}{2}[w-h]^T\Sigma^{-1}[w-h]\right).
\end{align}$$
...
$$\begin{align}
p(y|\theta) = \frac{1}{|2\pi(C+A\Lambda A^T)|^{1/2}}\exp(-\frac{1}{2}|y-\mu(\theta)|^T(C+A\Lambda A^T)^{-1}|y-\mu(\theta)|)
\end{align}$$

## Kernel trick

The problem with our model is that it is not flexible enough. To overcome this problem, we first project our model into a higher dimensional space using a basis function
