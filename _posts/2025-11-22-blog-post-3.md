...
There are a wide variety of Gaussian process python libaries with different implementations. The greatest challenge to computing a GP comes from inverting the covariance matrix $$C+A\Lambda A^T$$ in equation \@ref(eq:GPlikelihood), which has the computational complexity of $$\mathcal{O}(n^3)$$. Hence, the aim of the game is to reduce the complexity down. Some implementations is able to do this by using certain mathematical property's under special conditions.
