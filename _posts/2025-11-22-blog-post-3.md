...
$$\begin{align}
\mathcal{L}(d|\theta,\sigma, w) = \frac{1}{(2\pi C)}\exp(-\frac{1}{2}|y-\mu(\theta) - A*w|^T C^{-1}|y-\mu(\theta)-A*w|)
\end{align}$$

Next, we put a zero mean Gaussian prior with convariance matrix $$\Sigma$$ on the weights
...
$$\begin{align}
p(y|X) = \int^\inf_{-\inf}\frac{1}{2\pi\Sigma}\exp(-\frac{w^T\Sigma w}{2})\frac{1}{(2\pi C)}\exp(-\frac{1}{2}|y-\mu(\theta)-A*w|^TC^{-1}|y-\mu(\theta)-A*w|)
\end{align}$$
Let $$h=\Sigma A^T C^{-1}$$
$$\begin{align}
