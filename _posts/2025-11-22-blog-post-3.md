...
This is generalisation of the radial basis function kernel. The parameter $$\mu$$ controls the smoothness of the function, with $$\mu \rightarrow \infty$$ case being equivalent to the radial basis function kernel. $$\mu=3/2$$ is once differentiable and $$\mu=5/2$$ is twice differentiable.

$$\begin{align}
k(x_i, x_j) = \frac{1}{\Gamma(\mu)s^{\mu-1}}\left(\frac{\sqrt{2\mu}(x_i-x_j)}{l}\right)^\mu K_\mu(\frac{\sqrt{2\mu}(x_i-x_j)}{l})
\end{align}$$

## Exp-Sine-Squared (periodic) kernel
As the name suggests, this kernel allows functions to be periodic.

$$\begin{align}
k(x_i, x_j) = \exp\left(-\frac{}{}\right)
\end{align}$$
