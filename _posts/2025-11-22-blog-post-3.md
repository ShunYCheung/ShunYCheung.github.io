...
A Gaussian processes assign a prior probability to each of the infinite number of possible functions. We are not interested in having an infinitely many possible functions, but want to incorporate our observational data. Hence, we condition the predictions using the observational data we have. 
...
$$\begin{align}
k(x_i, x_j) = \exp(-\frac{(x_i-x_k)^2}{2l^2}) 
\end{align}$$

## Matern kernel

This is generalisation of the radial basis function kernel. It is also stationary. The parameter $$\nu$$ controls the smoothness of the function, with $$\nu \rightarrow \infty$$ case being equivalent to the radial basis function kernel. $$\nu=3/2$$ is once differentiable and $$\mu=5/2$$ is twice differentiable. The kernel can be useful if you don't want your functions to be smooth. You can have a situation where you data appears discontinuous, and the need of a non-smooth function.   
...
As the name suggests, this kernel allows functions to be periodic. This parameter $$p$$ controls the periodicity and the length parameter $$l$$ controls the length scale.
