...
To visulaise how we can project our data from a lower dimension to a higher dimension, we start with the simple case of 1D to 2D. We see that in 1D, the blue and red points are not linearly separable. But in the 2D case. it is linear separable as you can draw a line at y=1 and separate the blue and red dots. 

![]({{ '/assets/images/kerneltrick.png' | relative_url}})

As we project the data to higher and higher dimensions, we need to construct an ever bigger design matrix, which becomes computationally intractable as we increase the dimension. However, due [Mercer's theorem](https://en.wikipedia.org/wiki/Mercer%27s_theorem), where any inner product of matrices can be replaced by a function as long as it returns a positive semi-definite matrix. This function is the kernel function
