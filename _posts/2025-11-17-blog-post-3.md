...
$$\begin{align}
w \sim \mathcal{N}(0, \sum)
\end{align}$$

and hence the prior can be written as

$$\begin{align}
p(\vec{w}) = \frac{1}{2\pi\Lambda} \exp\left(-\frac{1}{2}\vec{w}^T\sum\vec{w} \right)
\end{align}$$

According the Bayes' theorem, we can write the posterior as

$$\begin{align}
p(\vec{w}|y, A) \propto \exp\left(-\frac{1}{2}|y-A\vec{w}|^T C^{-1}|y-A\vec{w}|\right)\exp\left(-\frac{1}{2}\vec{w}^T\sum \vec{w} \right)
\end{align}$$
...
$$\begin{align}
p(\vec{w}|y, A) \propto \exp\left(-\frac{1}{2}|\vec{w}-\hat{\vec{w}}|^T \left(\frac{1}{\sigma^2}AA^T+\sum^{-1}\right)|\vec{w}-\hat{\vec{w}}|\right)
\end{align}$$
