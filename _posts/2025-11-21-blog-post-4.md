...
The graph below visually demonstrates how the coupling layer works ![(Credit: Ivan *et al.* (2019))]({{ '/assets/images/coupling_flows.png' | relative_url}}). In this particular transformation, the Jacobian will be upper tringular:
...
Now that we introduce coupling flows, a question that natually arises is what should the coupling function $$h$$ be? Two architectures called
nonlinear independent component (NICE) proposed by Dinh *et al*. (2015) and real non-volume preserving (R-NVP) flows has two simple functions. The first is the additive coupling function:
...
where $$T$$ is known as a translation function. The second is the affine coupling function:
...
where $$T$$ is the same translation function as before and $$S$$ is a scaling function. 
...
- [Normalizing Flows](https://grishmaprs.medium.com/normalizing-flows-5b5a713e45e2)
