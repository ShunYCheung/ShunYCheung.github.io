...
$$\begin{equation}
p_Y(y) = p_X(g^{-1}(y))\left|\frac{\partial g(g^{-1}(x))}{\partial x}\right|.
\end{equation}$$

where the term $$\left|\frac{\partial g(g^{-1}(x))}{\partial x}\right|$$ is the Jacobian. One job for the Jacobian is to ensure the new probability density *normalises* to 1. The new probability density function $$p_Y(y)$$ is called a *pushforward* of the density $$p_X$$.
...
$$\begin{algin}
\log p_Y(y|\theta) = \log p_X(g^{-1}(y|\theta)|\phi) + \log\left|\det \frac{\partial g(g^{-1}(x))}{\partial x} \right|
\end{align}$$
